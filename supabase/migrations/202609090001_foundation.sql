-- Phase 1A approved foundation. Applied only by the checksummed migration runner.
-- Seven public application tables. Private run metadata is infrastructure only.
create schema if not exists car_show_private;
revoke all on schema car_show_private from public, anon, authenticated;
create table car_show_private.import_runs (
...
revoke all on car_show_private.import_runs from public, anon, authenticated;
...
create table public.events (
  ...
  -- Allocator locks this row; import seeds MAX(car_number)+1.
...
create table public.score_history (
  ...
  -- Append-only trigger is defined in the following migration; client roles are deferred.
...
-- No role assumptions or permissive RLS policies. No client writes or RPC grants.
revoke all on public.events, public.participants, public.cars,
  public.event_registrations, public.scores, public.profiles, public.score_history
