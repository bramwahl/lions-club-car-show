-- Atomic authenticated score/history services. All attribution is resolved in the database.
create function car_show_private.save_score(p_registration uuid,p_section text,p_values jsonb,p_request uuid,p_expected jsonb,p_admin boolean)
...
begin
  ...
  for field in select jsonb_object_keys(allowed) loop
    ...
    if raw is null or raw='null'::jsonb or (jsonb_typeof(raw)='string' and btrim(value_text)='') then
      ...
      if jsonb_typeof(raw) not in ('number','string') or value_text !~ '^[0-9]+$' or length(value_text)>10 then
...
revoke all on function car_show_private.save_score(uuid,text,jsonb,uuid,jsonb,boolean) from public,anon,authenticated;
create function public.submit_section(p_registration_id uuid,p_section text,p_values jsonb,p_request_id uuid) returns uuid
language sql security definer set search_path = '' as $$
  select car_show_private.save_score(p_registration_id,p_section,p_values,p_request_id,null,false);
$$;
create function public.admin_quick_edit(p_registration_id uuid,p_values jsonb,p_expected jsonb,p_request_id uuid) returns uuid
language sql security definer set search_path = '' as $$
  select car_show_private.save_score(p_registration_id,null,p_values,p_request_id,p_expected,true);
$$;
create function public.submit_lions_choice(p_registration_id uuid) returns integer
language plpgsql security definer set search_path = '' as $$
declare reg public.event_registrations; votes integer;
begin
  perform car_show_private.require_staff();
  select * into reg from public.event_registrations where id=p_registration_id for update;
  if reg.id is null or not exists(select 1 from public.events where id=reg.event_id and judging_open and legacy_source_key is null) then
    raise exception 'Voting is unavailable' using errcode='42501';
  end if;
  update public.event_registrations set lions_choice_votes=lions_choice_votes+1 where id=reg.id returning lions_choice_votes into votes;
  return votes;
end $$;
revoke all on function public.submit_section(uuid,text,jsonb,uuid),public.admin_quick_edit(uuid,jsonb,jsonb,uuid),public.submit_lions_choice(uuid) from public,anon,authenticated;
grant execute on function public.submit_section(uuid,text,jsonb,uuid),public.admin_quick_edit(uuid,jsonb,jsonb,uuid),public.submit_lions_choice(uuid) to authenticated;
