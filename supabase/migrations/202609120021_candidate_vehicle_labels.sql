-- Approved candidate vehicle labels; excludes car IDs and contact fields.
-- Approved limited exact-key candidate lookup; no contact projection.
...
declare cfg car_show_private.registration_portal; person uuid; matches integer; tok uuid; car uuid; request_id uuid; existing public.registration_requests;
 fingerprint text; item jsonb; ids uuid[]; selected uuid[]:='{}'; details jsonb; new_cars jsonb; event_name text; candidate_ids uuid[]; candidate uuid; candidates jsonb:='[]'::jsonb; entry jsonb;
begin
 ...
 if p_action='lookup' then
  if length(btrim(coalesce(p_data->>'name',''))) < 2 and length(btrim(coalesce(p_data->>'contact',''))) < 5 then return '{"matched":false}'::jsonb; end if;
  select array_agg(id) into candidate_ids from (
   select id from public.participants where
   (btrim(coalesce(p_data->>'name',''))='' or lower(btrim(name))=lower(btrim(p_data->>'name')) or lower(regexp_replace(btrim(name),'^.*[[:space:]]',''))=lower(btrim(p_data->>'name')))
   and (btrim(coalesce(p_data->>'contact',''))='' or
    (position('@' in p_data->>'contact')>0 and lower(btrim(email))=lower(btrim(p_data->>'contact'))) or
    (length(regexp_replace(p_data->>'contact','[^0-9]','','g'))>=10 and regexp_replace(regexp_replace(phone,'[^0-9]','','g'),'^1([0-9]{10})$','\1')=regexp_replace(regexp_replace(p_data->>'contact','[^0-9]','','g'),'^1([0-9]{10})$','\1')))
   order by name,id limit 6
  ) limited;
  if coalesce(cardinality(candidate_ids),0)=0 then return '{"matched":false}'::jsonb; end if;
  if cardinality(candidate_ids)>5 then return '{"matched":false,"error":"Several people match. Add your full name or phone/email to narrow the search."}'::jsonb; end if;
  foreach candidate in array candidate_ids loop
   insert into car_show_private.registration_matches(event_id,participant_id) values(cfg.event_id,candidate) returning token into tok;
   entry:=jsonb_build_object('token',tok,'participant',(select jsonb_build_object('name',name,'city',city,'state',state) from public.participants where id=candidate),'vehicles',coalesce((select jsonb_agg(jsonb_build_object('year',c.year,'make',c.make,'model',c.model) order by c.year,c.make,c.model) from public.cars c where c.participant_id=candidate),'[]'::jsonb));
   candidates:=candidates||jsonb_build_array(entry);
  end loop;
  if cardinality(candidate_ids)=1 and btrim(coalesce(p_data->>'name',''))<>'' and btrim(coalesce(p_data->>'contact',''))<>'' then
   return public.registration_portal(p_secret,p_client,'choose',jsonb_build_object('token',tok));
  end if;
  return jsonb_build_object('matched',false,'candidates',candidates);
