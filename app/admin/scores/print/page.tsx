...
export default async function ScorePrint({searchParams}:{searchParams:Promise<{event?:string}>}){
 ...
 const insights=dashboardInsights(rows,await priorRegistrations(client,earlier.map(e=>e.id)),earlier.length>0,await localLeftIds());
 ...
 const ids=rows.flatMap(r=>r.score?[r.score.id]:[]);const names=new Map<string,Set<string>>();
 for(let start=0;start<ids.length;start+=100){for(let offset=0;;offset+=500){const {data,error}=await client.from('score_history').select('judge_name_snapshot,section').in('score_id',ids.slice(start,start+100)).eq('action_type','section_submission').order('id').range(offset,offset+499);if(error)throw new Error('Judge attribution could not be loaded.');for(const r of data){const name=r.judge_name_snapshot||'Unattributed submission';const sections=names.get(name)??new Set<string>();if(r.section)sections.add(r.section);names.set(name,sections);}if(data.length<500)break;}}
 return <FullScoreReport dashboard={insights} attendance={attendance} filename={reportTitle(`${event.event_year} ${event.name} - Zionsville Lions Club`,'Event Summary')} event={event} rows={rows} judges={[...names].sort(([a],[b])=>a.localeCompare(b)).map(([name,sections])=>({name,sections:Object.entries(sectionLabels).filter(([key])=>sections.has(key)).map(([,label])=>label)}))} generated={new Intl.DateTimeFormat('en-US',{dateStyle:'medium',timeStyle:'short',timeZone:'America/Indiana/Indianapolis'}).format(new Date())}/>;
}
