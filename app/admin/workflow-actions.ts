...
export async function saveCar(_state:FormState,form:FormData):Promise<FormState>{const {client}=await requireStaff(true);let participant='';try{participant=id(form,'participant');const yearText=text(form,'year',4),year=Number(yearText);const values={color:text(form,'color',80).trim()||null,year,make:text(form,'make').trim(),model:text(form,'model').trim(),notes:text(form,'notes',5000)};if(!/^\d{4}$/.test(yearText)||year<1900||year>2155||!values.make||!values.model)return {message:'Enter a four-digit year, make and model.'};
...
export async function arriveCar(_state:FormState,form:FormData):Promise<FormState>{
 ...
 try{const color=text(form,'color',80).trim();if(!color)return {message:'Enter the car color to complete check-in.'};const result=await client.rpc('admin_arrival',{p_event:id(form,'event'),p_participant:id(form,'participant'),p_car:id(form,'car'),p_payment:text(form,'payment',10),p_color:color});if(result.error)return {message:'Check-in failed. Confirm the selected event, car and payment status.'};registration=result.data;}
 catch{return fail;}
