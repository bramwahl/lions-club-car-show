...
export async function saveCar(_state:FormState,form:FormData):Promise<FormState>{const {client}=await requireStaff(true);let participant='';try{participant=id(form,'participant');const yearText=text(form,'year',4),year=Number(yearText);const values={year,make:text(form,'make').trim(),model:text(form,'model').trim(),notes:text(form,'notes',5000)};if(!/^\d{4}$/.test(yearText)||year<1901||year>2155||!values.make||!values.model)return {message:'Enter a four-digit year, make and model.'};
