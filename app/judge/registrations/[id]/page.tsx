...
export default async function JudgingDetail({params}:{params:Promise<{id:string}>}){
 ...
 if(role==='admin'){
  ...
  if(!car){const r=data.registration;car={id,car_number:'',vehicle_year:r.vehicle_year,vehicle_make:r.vehicle_make,vehicle_model:r.vehicle_model,vehicle_color:r.vehicle_color,owner_name:r.participant.name,owner_city:r.participant.city,owner_state:r.participant.state,event_name:data.event.name,event_year:data.event.event_year,historical:!!data.event.legacy_source_key,judging_open:data.event.judging_open,progress:Number(score?.progress_percentage??0),sections:Object.fromEntries(Object.entries(SECTIONS).map(([section,fields])=>[section,Object.keys(fields).every(field=>score?.[field]!=null)]))};}
