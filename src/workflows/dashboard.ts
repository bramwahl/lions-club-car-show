...
export function dashboardInsights(rows:Registration[],prior:PriorRegistration[],hasBaseline:boolean){
 ...
 return {total:registered.length,checked:checked.length,paid:checked.filter(r=>r.payment_status==='Paid').length,judged:checked.filter(r=>r.status==='Judged').length,returning:hasBaseline?returning:null,newCars:hasBaseline?registered.length-returning:null,locations:top(r=>[r.participant.city?.trim()||'Unknown city',r.participant.state?.trim()||'Unknown state'].join(', ')),makes:top(r=>r.vehicle_make.trim()||'Unknown make')};
}
