...
export async function rpc<T>(client:SupabaseClient,name:string,args?:Record<string,unknown>):Promise<T>{const {data,error}=await client.rpc(name,args);if(error)throw new Error('The requested data could not be loaded. Please try again.');return data as T;}
...
export async function registrationDetail(client:SupabaseClient,id:string){
