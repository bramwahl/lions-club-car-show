...
import { awardsFor,registrations,rpc,localAwardInputs } from './data';
...
export async function awardResults(client:SupabaseClient,event:string){
 const [inputs,rows,events,choice]=await Promise.all([localAwardInputs(client,event),registrations(client,event),rpc<EventRow[]>(client,'admin_directory',{p_kind:'events'}),choiceConfirmation(client,event)]);
 const left=await localLeftIds();const skips=await localAwardSkips(event);const awards=calculateAwards(inputs,skips);const ties=awardTies(inputs,awards,skips);
 ...
 return {adjustments:skips.map(s=>{const row=rows.find(r=>r.car_id===s.car_id);return {...s,number:row?.car_number??null,vehicle:row?`${row.vehicle_year} ${row.vehicle_make} ${row.vehicle_model}`:'Car unavailable',result:awards.find(a=>a.car_id===s.car_id)?.award??null};}),awards,ties,stats:statistics(rows),registrationIds:Object.fromEntries(rows.map(r=>[r.car_id,r.id])),repeatKeys:repeatedAwardKeys([...awards,...ties.flatMap(tie=>tie.cars.map(car=>({car_id:car.car_id,award:tie.award})))],past),historyAvailable:previous.length>0,confirmationAvailable:choice.available,confirmedId:choice.winner&&!left.includes(choice.winner.registration_id)?choice.winner.registration_id:null,lions:rows.filter(r=>r.lions_choice_votes>0&&!left.includes(r.id)).sort((a,b)=>b.lions_choice_votes-a.lions_choice_votes).map(r=>({id:r.id,eligible:!!r.car_number&&['Checked-in','Judged'].includes(r.status??''),number:r.car_number,vehicle:`${r.vehicle_year} ${r.vehicle_make} ${r.vehicle_model}`,votes:r.lions_choice_votes})),updated:new Date().toISOString()};
}
