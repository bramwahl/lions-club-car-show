...
export function awardTies(inputs:readonly AwardInput[],awards:readonly Award[],skips:readonly {car_id:string;award:string}[]=[]):AwardTie[]{
 ...
 for(const winner of awards){
  if(!winner.award.startsWith('Top 40:')){
   ...
   const cars=[...remaining.values()].filter(car=>!skips.some(s=>s.car_id===car.car_id&&s.award===winner.award)&&(!className||classification(car.vehicle_year)===className)&&phpNumericCompare(car[field],winner[field])===0);
