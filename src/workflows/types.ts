...
export type Car={id:string;legacy_car_id:string|null;participant_id:string;year:number;make:string;model:string;notes:string|null};
export type Score=ScoreInputs & Totals & {id:string;event_registration_id:string;progress_percentage:number};
export type Registration={id:string;event_id:string;car_id:string;participant_id:string;car_number:string|null;vehicle_year:number;vehicle_make:string;vehicle_model:string;status:string|null;payment_status:string;classification:string|null;lions_choice_votes:number;participant:Participant;score:Score|null};
